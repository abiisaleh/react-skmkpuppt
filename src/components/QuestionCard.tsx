...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val) {
      ...
      if (year !== 2026) {
        setDateError('Tanggal penerimaan layanan harus berada pada tahun 2026.');
      } else {
        setDateError(null);
      }
    } else {
  ...
  return (
    <motion.div
      ...
      className="w-full bg-white rounded-2xl border border-slate-100 p-6 sm:p-8 kpu-card-glow"
    >
      {/* Category Header (Optional indicator only) */}
      {question.optional && (
        <div className="flex justify-end mb-4">
          <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-400">Opsional</span>
        </div>
      )}
      ...
      <div className="mt-8">
        {question.type === 'select' && (
          <div className="relative">
            ...
            >
              <option value="" disabled>-- Silakan Pilih Satuan Kerja --</option>
                ...
                >
                  ...
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center border transition-all ${
                      isSelected ? 'border-kpu-red bg-kpu-red text-white' : 'border-slate-300 bg-white'
                    }`}
                  >
            ...
            {question.options?.map((opt, idx) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ y: -1 }}
                  whileTap={{ scale: 0.99 }}
                  onClick={() => selectOptionWithDelay(score)}
                  className={`w-full text-left flex items-center justify-between p-4 rounded-xl border text-sm transition-all duration-150 cursor-pointer ${
                    isSelected
                ...
                >
                  <div className="flex flex-col">
                    <span className={`text-[15px] ${isSelected ? 'font-bold text-slate-900' : 'text-slate-800'}`}>
                      {opt}
                    </span>
                  </div>
                  <div
