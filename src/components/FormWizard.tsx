...
export const FormWizard: React.FC = () => {
    ...
    if (currentQuestion.type === 'date') {
      ...
      if (dateVal) {
        ...
        if (year !== APP_CONFIG.ACTIVE_YEAR) {
          showToast(`Tahun pelayanan harus bernilai ${APP_CONFIG.ACTIVE_YEAR}.`);
          return;
        }
      }
    }
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      console.error('Failed to submit via standard fetch, trying redirect style fallback...', error);
      setIsSubmitting(false);
      setIsSubmitted(true);
    }
  };
  ...
  const progressMetrics = (() => {
    ...
    const secQuestions = questions.filter(q => q.section === activeSec && isQuestionVisible(q));
    
    const currentStepInSec = secQuestions.findIndex(q => q.id === currentQuestion.id) + 1;
    const totalStepsInSec = secQuestions.length;

    const stepLabel = currentStepInSec > 0 && totalStepsInSec > 0
      ? `Langkah ${currentStepInSec} dari ${totalStepsInSec}`
      : '';
    
    const percent = totalStepsInSec > 0
  ...
  return (
    <div className="w-full max-w-2xl px-4 py-8 flex flex-col items-center">
      ...
      {!showWelcome && !isSubmitted && !isSubmitting && (
        ...
        >
          <div className="flex items-center justify-between text-xs font-semibold text-slate-500 mb-2">
            <span className="text-kpu-red uppercase tracking-wide font-extrabold text-[11px] bg-kpu-red-light px-2.5 py-0.5 rounded-full">{progressMetrics.activeSec}</span>
            <span className="text-kpu-orange text-[11px] font-bold">{progressMetrics.label} ({progressMetrics.percent}%)</span>
          </div>
        ...
        {isSubmitting ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-slate-500 mt-2 max-w-xs leading-relaxed">
              Data Anda aman dan sedang dikirim secara langsung ke sistem data terpadu KPU. Mohon tunggu sebentar.
            </p>
          </div>
        ) : isSubmitted ? (
          <SuccessStep answers={answers} onReset={handleReset} onShare={handleShare} />
        ) : showWelcome ? (
      ...
      {!isSubmitted && !isSubmitting && !showWelcome && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-md border-t border-slate-200/60 py-4 px-6 shadow-[0_-8px_30px_rgba(0,0,0,0.04)] animate-slide-up">
          <div className="max-w-2xl mx-auto flex items-center justify-between gap-4 w-full">
            ...
            >
              <span>{currentIndex === questions.length - 1 ? 'Kirim Tanggapan ✓' : 'Berikutnya'}</span>
